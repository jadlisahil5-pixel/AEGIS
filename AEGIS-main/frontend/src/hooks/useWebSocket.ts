...
import { normalizeWsEvent } from "@/lib/ws-utils";

type SubscriptionCallback = (data: Record<string, unknown>) => void;

export function useWebSocket(wsUrl: string = config.wsUrl) {
  const clientRef = useRef<Client | null>(null);
  const pendingRef = useRef<Map<string, SubscriptionCallback>>(new Map());
  const activeRef = useRef<Map<string, { callback: SubscriptionCallback; unsubscribe: () => void }>>(
    new Map(),
  );
