...
function AdminPortal() {
  ...
  // a fabricated x/y grid position.
  const markers: MapMarker[] = [
    ...
    ...hospitalRecords
      ...
      .map((h) => ({
        ...
        label: h.name,
        status:
          h.totalIcuBeds && h.availableIcuBeds !== undefined
            ? `${h.availableIcuBeds}/${h.totalIcuBeds} ICU`
            : undefined,
      })),
    // Only the real Ghaziabad signals — the legacy Bengaluru demo grid
    ...
    ...traffic.signals
      ...
      .map((s) => ({
        ...
        lng: s.lng,
        label: s.roadSegment,
        active: s.currentState === "GREEN_PRIORITY",
      ...
      {tab === "operations" && (
        <div className="space-y-6">
          ...
          <div className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
            ...
            >
              {/* Real Ghaziabad map. The route drawn here is a real, Google
                  Routes API-computed path (fetched via the backend proxy)
                  for the active incident's assigned ambulance — not a
                  hardcoded demo path. It only appears once both positions
                  are known and the backend has a routing key configured. */}
              <LiveMap
                className="h-[420px]"
                markers={markers}
